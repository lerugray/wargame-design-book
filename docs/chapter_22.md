...
AI has no taste. It will not tell you if a premise holds together or if a mechanic actually works at the table. It cannot read a combat results table and judge whether the friction matches the historical situation. Determining what works requires historical grounding, playtest mileage, and a clear view of what the simulation is supposed to accomplish. The system does not care about the outcome. You do.

Ask an AI to draft a concept from nothing and you receive a statistical average of the genre. Give it a mechanical prompt and it will output procedures that function, but they carry no design intent. Unedited rules read like they were compiled by people who have never resolved a CRT or tracked supply lines. The text looks serviceable until a player tries to use it. That is when the gaps show. Wargamers recognize this material quickly. They have spent years reading rulebooks and tearing down systems, and they know exactly what forced text looks like when it lands on the table.
...
If you choose to use AI, it only works as a force multiplier when you anchor it to your own research and design judgment. With that knowledge behind it, it lets a solo designer working limited hours finish work that would otherwise require a team or remain unfinished. Without it, you get text that reads exactly like you skipped the work entirely. The quality comes from the designer, not the tool.
...
The bottleneck moved. For most of my design career, the gate between an idea and a working prototype was coding ability. A Monte Carlo simulator, a browser-playable rulebook test, an OSINT terminal that pulls live conflict data: I could imagine all of these. I could not build them, because building them required programming skills I did not have and could not justify spending years acquiring. AI removed that gate. Now the constraint is the one it was always supposed to be: design judgment. Knowing what is worth building and what to refuse.
